...
type HandlerFuture = BoxFuture<'static, Result<Bytes>>;
type HandlerFn = dyn Fn(Bytes) -> HandlerFuture + Send + Sync;

#[async_trait]
pub trait HppHandler: Send + Sync + 'static {
    async fn on_hpp_message(&self, data: Bytes) -> Result<Option<Bytes>>;
    // underscore to silence "unused variable" warnings for the default impl
    async fn handle_method(&self, _method_id: u32, _body: Bytes) -> Result<Bytes> {
        Ok(Bytes::from(vec![]))
    }
}

pub struct SimpleHandler {
    // method_id -> async handler
    registry: Mutex<HashMap<u32, Arc<HandlerFn>>>,
}
...
impl SimpleHandler {
    ...
    {
        let boxed: Arc<HandlerFn> = Arc::new(move |b: Bytes| -> HandlerFuture {
            Box::pin(f(b))
        });
        self.registry.lock().insert(method_id, boxed);
    }
...
impl HppHandler for SimpleHandler {
    async fn on_hpp_message(&self, data: Bytes) -> Result<Option<Bytes>> {
        // Parse HPP + RMC
        let res = HppPacket::parse(data.clone().into())
            .and_then(|p| RmcMessage::parse(p.payload).map_err(|e| e.into()));

        match res {
            Ok(rmc) => {
                info!(method = rmc.method_id, "Received RMC message via HPP");

                // Clone the callback OUTSIDE the lock (drop guard before await)
                let cb = {
                    let reg = self.registry.lock();
                    reg.get(&rmc.method_id).cloned()
                };
