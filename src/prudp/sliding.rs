...
///
/// NAK semantics:
/// - We return `(base, mask)` where `base` is the **first missing** sequence.
/// - `mask` is a 32-bit bitmap where **bit 0 acknowledges `base + 1`**, bit 1 -> `base + 2`, etc.
///   (`base` itself is *not* represented in the mask.)
#[derive(Debug)]
pub struct RecvState {
    pub recv_base: u16,
    pub recv_buffer: BTreeMap<u16, Bytes>,
...
impl RecvState {
    ...
    pub fn offer(&mut self, seq: u16, payload: Bytes) -> Option<(u16, u32)> {
        ...
        // If we've just filled the next expected sequence, advance base as much as possible
        if self.recv_buffer.contains_key(&self.recv_base) {
            while let Some(_) = self.recv_buffer.remove(&self.recv_base) {
                self.recv_base = self.recv_base.wrapping_add(1);
            }
            return None; // no gap after advancing
        }

        // There's a gap: first missing is exactly recv_base.
        // Build mask where bit 0 => recv_base + 1, bit i => recv_base + 1 + i
        let base = self.recv_base;
        let mut mask: u32 = 0;
        for i in 0..32u16 {
            let s = base.wrapping_add(1 + i);
            if self.recv_buffer.contains_key(&s) {
                mask |= 1u32 << i;
            }
        }
...
impl SlidingWindow {
    ...
    /// Attach an RTT estimator (placeholder)
    pub fn attach_rtt(&mut self, _rtt: Arc<crate::rtt::RTT>) { /* store if needed */ }
    ...
    /// Mark a sequence as acknowledged; slide window forward and remove entries.
    pub fn mark_acked(&mut self, ack_seq: u16) {
        // cumulative ack: remove from send_base..=ack_seq
        let mut seq = self.send_base;
        while seq != ack_seq.wrapping_add(1) {
            self.sent_buffer.remove(&seq);
            seq = seq.wrapping_add(1);
    ...
    /// Ack a single sequence only (non-cumulative).
    pub fn mark_acked_single(&mut self, seq: u16) {
        if self.sent_buffer.remove(&seq).is_some() {
            // advance base while the lowest entries are gone
            while !self.sent_buffer.contains_key(&self.send_base) && self.send_base != self.next_seq {
                self.send_base = self.send_base.wrapping_add(1);
            }
        }
    ...
    /// Apply a multi-ack mask: bit i acknowledges (start + 1 + i).
    pub fn mark_acked_range(&mut self, start: u16, mask: u32) {
        for i in 0..32 {
            if ((mask >> i) & 1) != 0 {
                let s = start.wrapping_add(1 + i as u16);
                self.mark_acked_single(s);
            }
        }
    }
    ...
    /// Offer a packet to a specified stream (stream_id optional). Returns optional NAK info.
    pub fn offer_receive_stream(
        &mut self,
        stream_id: Option<u16>,
        seq: u16,
        payload: Vec<u8>
    ) -> (Option<Bytes>, Option<(u16,u32)>) {
        let sid = stream_id.unwrap_or(0);
        let state = self.recv_states.entry(sid).or_insert_with(|| RecvState::new(1));
        let bytes = Bytes::from(payload);
        let nak = state.offer(seq, bytes.clone());
        // Delivery path omitted here; integrate if needed.
        (None, nak)
    }
}
...
mod tests {
    ...
    fn test_receive_nak() {
        ...
        assert!(n2.is_some());
        if let Some((base, mask)) = n2 {
            assert_eq!(base, 2);          // first missing
            assert_eq!(mask & 0b1, 0b1);  // bit 0 => base+1 (=3) is present
        }
    }
}
